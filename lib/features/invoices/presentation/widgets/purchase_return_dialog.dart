...
import '../../application/purchase_return_service.dart';
...
class _PurchaseReturnDialogState extends ConsumerState<PurchaseReturnDialog> {
  ...
  Future<void> _submit() async {
    ...
    for (final item in widget.items) {
      ...
      lines.add(PurchaseReturnLine(invoiceItemId: item.id, quantity: q));
    }
    ...
    try {
      final db = ref.read(databaseProvider);
      await _service.recordPurchaseReturn(
        db,
        user: user,
    ...
    } on PurchaseReturnException catch (e) {
      if (!mounted) return;
      AppFeedback.error(context, '$e');
    } catch (e, st) {
      await AppLogger.record('Purchase return', error: e, stackTrace: st);
      if (!mounted) return;
        ...
        child: _loadingRemain
            ...
            : SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${l10n.originalPurchase}: ${widget.invoice.invoiceNumber}',
                      style: TextStyle(
                        fontSize: 13,
                        color: isDark
